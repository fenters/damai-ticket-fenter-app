...
class DamaiAppTicketRunner:
    ...
    def _perform_ticket_flow(self) -> bool:
        try:
            ...
            if not self._select_date():
                raise TicketRunnerError("未能选择目标日期")
    ...
    def _search_event(self, keyword: str) -> bool:
        driver = self._ensure_driver()
        ...
        try:
            # 3. 点击搜索后的结果
            search_results = WebDriverWait(driver, 3.0).until(
    ...
    def _select_date(self) -> bool:
        ...
        try:
            date_moons = WebDriverWait(driver, 3.0).until(
                EC.presence_of_element_located((By.ID, "cn.damai:id/hrv_canlendar"))
            )
        except TimeoutException:
            self._log(LogLevel.ERROR, "未能加载日期选择页面")
            return False
    ...
    def _select_session(self) -> None:
        ...
        # 直接点击目标索引元素就行了
        try:
            session_elem = container.find_element(By.XPATH, f'.//android.widget.LinearLayout[{self.config.session_index}]')
            session_elem.click()
        except Exception as e:
            self._log(LogLevel.ERROR, f"场次选择失败: {e}")
            return False
    ...
    def _select_price(self) -> None:
        ...
        # 直接点击目标索引元素就行了
        try:
            price_elem = container.find_element(By.XPATH, f'.//android.widget.FrameLayout[{self.config.price_index}]')    
            price_elem.click()
        except Exception as e:
            self._log(LogLevel.ERROR, f"票价选择失败: {e}")
            return False
    ...
    def _select_quantity(self) -> bool:
        driver = self._ensure_driver()
        try:
            quantity = getattr(self.config, 'ticket_quantity', 1)
            # 找到加号按钮
            plus_button = WebDriverWait(driver, 3.0).until(
                EC.element_to_be_clickable((By.ID, "cn.damai:id/img_jia"))
            )
            
            # 点击 ticket_quantity - 1 次
